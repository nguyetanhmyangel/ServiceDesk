...
(function($) {
    var options = {
        ...
        tooltipOpts: {
            content:
                "%s | X: %x | Y: %y.2", //%s -> series label, %x -> X value, %y -> Y value, %x.2 -> precision of X value, %p -> percent
            dateFormat: "%y-%0m-%0d",
            shifts: {
                x: 10,
                y: 20
            ...
            $(placeholder).bind("plothover",
                function(event, pos, item) {
                    if (item) {
                        ...
                        $tip.html(tipText).css({ left: tipPosition.x + to.shifts.x, top: tipPosition.y + to.shifts.y })
                            .show();
                    } else {
                        $tip.hide().html("");
                    }
